 ...
 */
@Repository
public interface TransportistaRepositori extends JpaRepository<TransportistaModel, Integer> {

    @Override
    List<TransportistaModel> findAll();

    @Query(value = "select * from bd_beneficio_cafetito.bd_transportista bt where bt.nombre_usuario  =:pcodigo", nativeQuery = true)
    TransportistaModel getTransportistaByCodigo(@Param("pcodigo") String codigo);
